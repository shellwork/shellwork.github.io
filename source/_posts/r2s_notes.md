---
title: 基于r2s软路由部署iStoreOS解决HHT验证问题的通用方案——小白逐步教程
...
- **友善r2s 软路由设备**（必须需要有一张 SD 卡用于存储 iStoreOS 系统，无SD卡无法使用）：这是一个入门级别软路由硬件，某东裸机不带电源全新240，作者闲鱼上169购得，性能大约跑满800M带宽网络，对于500M绰绰有余。有一个LAN口一个WAN口。
...
> 该部分视频教程详见（本文不再过多重复）：[bilibili_links](https://www.bilibili.com/video/BV1vv4y1D71K/?t=3.117207&spm_id_from=333.1350.jump_directly&vd_source=d03e61540959009532833f4cca457930)
...
### Part2: 修改硬件连接方式
  ...
  - 这里讲一下核心要点：我们网络最终接线结构是和前文提到的用户网络的拓扑图相同的。但是在安装部署过程中，各项软件的下载安装依然需要连接网络。为了方便广大读者直接在软路由上安装和配置（如果您会手动下包安装然后再手动上传也可），这里最好是先正常登录接入软路由局域网。 
  ...
  具体接线如图所示：![光猫接线](ca41bc0de468bca2a5818ccfe89ad40.jpg)
  光猫接线：白色线为光猫2.5g入网口
  ![r2s接线](03ce7d299962327d005d1d3761cb8a3.jpg)
  r2s接线：白色另外一头为光猫，这头接入Wan口；黄色接入Lan口，另外一头为路由器
  ![路由器接线](edbd6803cfc975d0a8b4c98bf5b6050.jpg)
  路由器接线：黄色接入路由器Wan口，其余接线为其他设备
